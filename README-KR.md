...
### 전문가의 디자인 판단을<br />코딩 에이전트가 반복해서 실행하도록.

<sub>StyleSeed는 <b>전문가를 대체하는 디자인 취향 모음이 아니라, 전문가의 디자인 판단을 코딩 에이전트가 반복해서 실행할 수 있게 만드는 시스템</b>입니다. Claude Code · Codex · Cursor용 오픈소스.</sub>
...
이는 프로젝트의 목표이며, 현재 전문가 수준의 결과를 보장한다는 뜻은 아닙니다.
현재 엔진은 선택한 규칙을 컴파일하고 프로젝트 결정을 보존하며 코드·렌더 검사를 지원합니다.
...
StyleSeed는 **74개 craft 규칙**과 설치된 워크플로가 80점 미만 화면을 수정하도록 지시하는
**0–100 채점 게이트**를 제공합니다. 규칙은 보편적 취향이 아니라 선택한 문법과 프로젝트
계약 안에서 해석해야 합니다. 마크다운만으로 배포를 차단하지 않으며, 80점은 전문가의 승인이 아닙니다.

| 검사 | 이유 |
|---|---|
| 선택하지 않은 기본 액센트 | 색의 역할을 의도적으로 결정하고 기록하기 위해 |
| UI 아이콘 자리에 이모지 🚗 🧺 ⭐ | 제멋대로 색 주입 + OS마다 다르게 렌더링 |
| 기능 카드마다 아이콘 칩 하나씩 | 1세대 AI 티 — 정보인 척하는 장식 |
| 같은 무게 카드의 균등 그리드 | 기계 조합 티 1위: 초점이 없음 |
| 의도하지 않은 단일 검정 배경 | 선택한 문법의 표면 체계 적용; 지원되는 명시적 예외는 구분 |
| 선택한 타입 스케일 밖의 크기 | 의도하지 않은 정보 위계 변화 방지 |
| 컴포넌트에 하드코딩된 hex | 토큰만 — 아니면 락이 화면 간에 유지 안 됨 |

<sub>기본값 금지만으론 부족합니다 — 옛 티를 막으면 에이전트는 새 유니폼으로 수렴하니까요. 그래서 설치된 워크플로는 룰을 체크리스트가 아니라 채점-수정 루프로 실행합니다.</sub>
...
**전문가의 결정을 반복 가능한 지침으로 옮깁니다. 하나의 취향이나 Toss 룩을 정답으로 삼지 않습니다.**<br />
...
- 전문가의 디자인 결정을 **Claude Code·Codex·Cursor**가 일관되게 적용하도록 만들고 싶은 팀
- 새 제품의 디자인 시스템을 정의하거나 기존 시스템을 보존하며 AI로 개발하고 싶은 분
...
토큰과 컴포넌트는 필요하지만 모든 디자인 결정을 설명하지는 않습니다. 에이전트에는
패턴을 언제 쓰는지, 왜 이 작업에 적합한지, 무엇이 승인된 선택인지, 예외를 어떻게 처리하는지도
필요합니다. 전문가의 판단과 실제 구현 재료가 함께 전달되어야 합니다.

StyleSeed의 목표는 의도·트레이드오프·승인 권한을 사람에게 남기면서 그 판단을 반복 가능하게
만드는 것입니다. 현재 엔진은 출발점입니다. 컴포넌트 문맥 실험과 독립적인 전문가 평가는
...
```
"컴포넌트마다 색을 새로 만들지 말고 승인된 시맨틱 역할을 사용한다"
"취향이나 템플릿이 아니라 작업 목적에 맞춰 그룹과 밀도를 결정한다"
"프로젝트의 타입·표면 체계를 화면 사이에서 보존한다"
"로딩·빈 상태·오류·복구도 구현 범위에 포함한다"
"지원하지 않는 선택은 보고하고, 디자인 시스템을 몰래 바꾸지 않는다"
```

StyleSeed는 유지 관리되는 지침을 문법·어댑터·도메인·프로젝트 선택으로 나누고 선택한
문맥만 에이전트에 전달합니다. 기본값을 보편적인 전문가 판단으로 취급하지 않습니다.
지원하지 않는 프로젝트 선택에는 검토가 필요하며, 임의의 스킨이나 컴포넌트 교체로 숨기지 않습니다.

핵심 판단은 **브랜드 독립적**이지만 모든 결과물에 같은 배치를 강제하지 않습니다.
`Core judgment × Output grammar × Surface adapter × Domain/Page × Brand recipe × Palette recipe × optional Style profile`로
...
패키지를 직접 검토해야 합니다. 코드·프롬프트·스크린샷·URL·브랜드 토큰·임의 필드는
거부합니다. 준비된 패키지는 로컬에만 남고 전송되지 않습니다. extension 소스의 개발용
learning bridge는 host-owned proof adapter가 검증될 때까지 비활성 상태여야 합니다. 이후에도
별도 1회용 grant로 연결된 클라이언트·모델에 정확한 패키지 하나만 노출할 수 있습니다.
CLI와 bridge 모두 레지스트리에 업로드하거나 코어 룰을 자동으로 바꾸지
않습니다.
...
**요약:** 컴포넌트 라이브러리는 구현 재료를 줍니다. StyleSeed는 기록된 디자인 결정과
이를 적용·검사하는 워크플로를 연결합니다. 목표는 **전문가 판단의 반복 실행**이지,
디자이너나 프로젝트의 컴포넌트 라이브러리를 대체하는 것이 아닙니다.
...
**Q: Cursor에서도 되나요?**
됩니다. 프로젝트에 `.cursorrules`를 설치하거나 Cursor의 스킬 선택기에서 StyleSeed를
호출하면 시각 작업이 `STYLESEED.md`와 컴파일된 룰을 읽습니다. `CLAUDE.md`는 Claude
Code용 진입점입니다.
