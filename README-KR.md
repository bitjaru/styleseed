...
```text
StyleSeed를 `npx skills add bitjaru/styleseed`로 설치하고 이 프로젝트에 설정해줘. 내가 무엇을 만드는지와 꼭 필요한 디자인 선택만 물어봐. 결과물의 목적에 맞는 출력 문법·브랜드 레시피·팔레트 posture를 고르고, 키 컬러가 있으면 OKLCH ramp·어울리는 동반색·표면·시맨틱 역할·대비 증거까지 생성해. 특정 회사를 복제하지 마. 내 레퍼런스가 기존 문법에 없으면 프로젝트 전용 문법으로 만들어. 세 가지 방향안, 이미지·영상 작업, 실제 인터랙션 릴까지 필요하면 Claude Code의 `/ss-studio` 또는 Codex의 `$ss-studio`를 사용해. 확정된 방향은 resolve한 뒤 하나의 명확한 초점을 가진 결과물로 만들고, 코드 점수를 80점 이상으로 고쳐 실제 화면과 동작을 검증한 다음 보여줘.
```
...
어댑터, 컨텍스트·레퍼런스 컴파일러, 키 컬러를 OKLCH ramp·표면·동반색·시맨틱 역할·대비 증거로 바꾸는 팔레트 엔진(8개 기본 posture 포함), 48개 컴포넌트, 23개 `ss-*` 워크플로우 스킬이 LLM에게 데이터가
아니라 판단을 가르칩니다:
...
| `/ss-learn` | 사람이 채택한 디자인 교훈을 민감정보가 최소화된 로컬 후보로 기록·검토하고, 전송 없는 opt-in 공유 패키지 생성 |
...
### 사용자를 학습하지 않고, 채택된 교훈만 학습

`/ss-learn`은 명시적으로 요청했을 때만 로컬에서 작동합니다. 사람이 캡처를 요청해야
후보를 만들고, 채택과 공유 패키지 준비에는 각각 별도 승인이 필요합니다. 코드·프롬프트·
스크린샷·URL·신원·브랜드 토큰·임의 필드는 거부합니다. 선택형 로컬 MCP는 프로젝트를
스캔하지 못하며 별도의 1회용 승인 grant가 있는 정확한 패키지 하나만 연결된 클라이언트·
모델에 보여준 뒤 grant를 소모합니다. CLI와 MCP 모두 레지스트리에 업로드하거나 코어 룰을
자동으로 바꾸지 않습니다.

### Codex 플러그인 패키지

저장소에는 검증된 `.codex-plugin/plugin.json`, 동일한 정본 스킬 23개, 승인형 로컬 MCP가
포함됩니다. 현재는 로컬 테스트와 향후 플러그인 디렉터리 배포를 위한 패키지 경계이며,
공개 디렉터리에서 설치본을 별도로 검증하기 전에는 출시됐다고 주장하지 않습니다. 지금의
공개 설치 경로는 계속 `npx skills add bitjaru/styleseed`입니다.
