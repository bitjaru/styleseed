...
```text
StyleSeed를 `npx skills add bitjaru/styleseed`로 설치하고 이 프로젝트에 설정해줘. 내가 무엇을 만드는지와 꼭 필요한 디자인 선택만 물어봐. 결과물의 목적에 맞는 출력 문법·브랜드 레시피·시맨틱 팔레트를 고르되 특정 회사를 복제하지 마. 내 레퍼런스가 기존 문법에 없으면 프로젝트 전용 문법으로 만들어. 세 가지 방향안, 이미지·영상 작업, 실제 인터랙션 릴까지 필요하면 Claude Code의 `/ss-studio` 또는 Codex의 `$ss-studio`를 사용해. 확정된 방향은 resolve한 뒤 하나의 명확한 초점을 가진 결과물로 만들고, 코드 점수를 80점 이상으로 고쳐 실제 화면과 동작을 검증한 다음 보여줘.
```
...
어댑터, 컨텍스트·레퍼런스 컴파일러, 8개 시맨틱 팔레트, 48개 컴포넌트, 22개 `ss-*` 워크플로우 스킬이 LLM에게 데이터가
아니라 판단을 가르칩니다:
